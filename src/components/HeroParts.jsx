...
import { SOCIAL_SOURCE, SOCIAL_HANDLE } from "@/lib/social";
...
/**
 * Piezas del hero de servicios que comparten el hero habitual (`Hero`) y los
 * de campaña (`halloween/*`): la lista de accesos con sus reglas reales
 * (domicilios y reservas según la configuración de la tienda, juegos según
 * si es mesa), la rejilla de tarjetas y el bloque de Instagram con el 10%.
 *
 * Un hero de campaña cambia la escena y la marca, no lo que el local ofrece:
 * por eso ninguno debe reconstruir esta lista a mano. Las tarjetas pintan
 * con tokens (primary, secondary), así que el tema de la campaña las tiñe
 * solo.
 */
...
/**
 * Los accesos del hero y el manejador de los anclas internos (#carta,
 * #sala-vip...). Devuelve `featuredCount` para que la rejilla sepa dónde
 * poner el rótulo "TAMBIÉN" en móvil.
 */
export function useHeroServices() {
  ...
  return { services, featuredCount, handleAnchorClick };
}

/**
 * Servicios. Móvil: tres tarjetas grandes y cuatro fichas bajo "TAMBIÉN".
 * Escritorio: los siete en rejilla de cuatro columnas.
 */
export function HeroServiceGrid({ services, featuredCount, onAnchorClick }) {
  return (
...
/**
 * Síguenos. Instagram no es un servicio más del local: es la cuenta que se
 * quiere hacer crecer, así que va en fila propia y de ancho completo, con el
 * @ grande — es lo que la gente teclea luego en la aplicación si no toca el
 * enlace. TikTok se queda al lado como icono para no partir la atención en
 * dos.
 *
 * Debajo, el motivo: un botón sin razón se mira y no se toca. El 10% por
 * historia ya vive en #descuento-redes, pero ese bloque está a un scroll de
 * aquí. La campaña de Amor y Amistad daba ese motivo en su propio hero (el
 * shot gratis); al apagarla el 2026-09-21, la carta habitual se quedó sin él.
 *
 * Destaca por contraste y tamaño, no por color: el magenta y el cyan del hero
 * siguen significando carta y domicilios, y darle uno a Instagram lo haría
 * pasar por un servicio del local.
 *
 * `className` coloca el bloque dentro de cada hero (en el habitual va a la
 * derecha en escritorio).
 */
export function HeroSocial({ onAnchorClick, className = "" }) {
  return (
