...
// Predefined positions for each text element (percentage-based)
const positions = [
...
const FloatingText = ({ message, position, index }) => {
  ...
  // Each text appears at a different scroll threshold
  const threshold = (index + 0.5) / (messages.length + 1);

  // Raw transforms based on scroll
    ...
    >
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <span
          ...
          style={{
            color: "rgba(232, 64, 128, 0.35)",
            textShadow: "0 0 12px rgba(255, 100, 150, 0.15)",
          }}
        >
          {message.text}
        </span>
