...
const FlavorCard = ({ flavor, index }) => (
  ...
  >
    ...
    <div
      className={`absolute -inset-1 bg-linear-to-r ${flavor.glowGradient} rounded-3xl blur-lg opacity-40 group-hover:opacity-70 transition-opacity duration-500`}
    ></div>

    <div
      className={`relative bg-dark border-2 ${flavor.borderColor} rounded-3xl overflow-hidden h-full flex flex-col transition-all duration-500 hover:shadow-2xl ${flavor.shadowColor}`}
    >
      {/* Animated background pattern */}
      <div className="absolute inset-0 opacity-10">
        <div
          className={`absolute top-0 right-0 w-32 h-32 ${flavor.bgColor} rounded-full filter blur-3xl animate-pulse`}
        ></div>
        <div
          className={`absolute bottom-0 left-0 w-24 h-24 ${flavor.bgColor} rounded-full filter blur-2xl animate-pulse`}
      ...
      <div className="p-4 sm:p-6 flex flex-col grow relative z-10">
        <div
          className={`w-12 h-12 sm:w-16 sm:h-16 bg-linear-to-br ${flavor.gradient} rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4 shadow-xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-500`}
        >
          <flavor.icon className="text-dark" size={24} />
        </div>

        <h3
          className={`text-lg sm:text-2xl font-black mb-2 ${flavor.textColor} transition-all duration-300`}
        >
          {flavor.name}
        </h3>
        ...
        <div className="mt-auto pt-3 sm:pt-4 border-t border-gray/20">
          <div className="flex items-center gap-1 sm:gap-2">
            <Zap className={flavor.textColor} size={14} />
            <span
              className={`text-xs sm:text-sm font-semibold ${flavor.textColor}`}
            >
              Electrolit + Bonfiest
            </span>
          </div>
  ...
  const flavors = [
    {
      ...
      name: "Maracuyá",
      description:
        "El poder tropical del maracuyá combinado con la hidratación del Electrolit y el boost de Bonfiest. ¡Revive al instante!",
      icon: Star,
    ...
    },
    {
      ...
      name: "Fresa",
      description:
        "Dulzura refrescante de fresa que te hará olvidar la noche anterior. Hidratación y energía en cada sorbo.",
      icon: Heart,
    ...
    },
    {
      ...
      name: "Coco",
      description:
        "La frescura del coco te transporta a la playa mientras tu cuerpo se recupera. Hidratación tropical máxima.",
      icon: Sparkles,
    ...
    },
    {
      ...
      description:
        "Explosión cítrica que despierta todos tus sentidos. Vitaminas y electrolitos para una recuperación express.",
      icon: Zap,
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        {/* Header Section - Extra llamativo */}
        <motion.div
