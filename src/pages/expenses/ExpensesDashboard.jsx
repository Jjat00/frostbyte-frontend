    ...
    <div className="space-y-6">
      ...
      {stats?.by_category && stats.by_category.length > 0 && (
        <motion.div
          ...
          className="bg-dark-secondary rounded-xl p-5 border border-gray/10"
        >
            ...
            {pendingData.expenses.slice(0, 5).map((expense) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-4 p-3 bg-dark rounded-lg hover:bg-gray/10 transition-colors"
                >
                  <div className={`p-2 rounded-lg border ${colorClass}`}>
                    <IconComponent className="w-4 h-4" />
                  </div>
      ...
      {(!stats?.by_category || stats.by_category.length === 0) &&
        (!pendingData?.expenses || pendingData.expenses.length === 0) && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-dark-secondary rounded-xl p-8 border border-gray/10 text-center"
          >
