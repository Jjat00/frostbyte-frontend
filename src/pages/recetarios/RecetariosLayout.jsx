      ...
      <aside className="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-white/[0.07] bg-dark md:flex">
        <div className="p-5 border-b border-white/[0.07]">
          ...
          >
            ...
            />
            <div>
              <h1 className="font-display text-[0.88rem] font-semibold tracking-[0.16em] text-light transition-colors group-hover:text-light/70">
                FROSTBYTE
              </h1>
              <p className="text-xs text-gray">Recetarios</p>
            </div>
          </NavLink>
        </div>
          ...
          {navItems
            .filter(
              (item) =>
                ...
                item.path !== "/" &&
                item.path !== "/musica" &&
                item.path !== "/juegos-admin"
            )
            .map((item) => {
              const active =
                !item.external && isActivePath(item.path, item.end);
...
export default RecetariosLayout;
