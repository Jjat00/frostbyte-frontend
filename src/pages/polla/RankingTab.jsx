...
import { RANKING, MY_STATS, PRIZE } from "@/data/mundial2026";
...
const RankingTab = () => {
  ...
  // Sustituye "Tú" por el nombre real de la sesion
  const rows = RANKING.map((r) =>
    r.isYou ? { ...r, name: firstName, avatar: customer?.avatar_url } : r
  );
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm leading-snug text-gray">
            Compite por{" "}
            <span className="font-bold text-secondary">{PRIZE.champion}</span>.
        ...
        <div className="mb-6 flex items-center gap-4 rounded-2xl border border-primary/30 bg-linear-to-r from-primary/10 to-secondary/5 p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-gray">
              Vas en el puesto {me.pos} de {MY_STATS.total}. ¡Sigue sumando!
            </p>
          </div>
      ...
      <div className="mb-6 flex items-center gap-3 rounded-2xl border border-secondary/20 bg-secondary/[0.05] px-4 py-3">
        ...
        <div>
          <p className="text-sm font-bold text-light">
            Premio al campeón: {PRIZE.champion}
          </p>
          <p className="text-[11px] text-gray">{PRIZE.note}</p>
        </div>
      </div>
        ...
        {rows.map((r) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <Avatar name={r.name} src={r.avatar} size={36} highlight={r.isYou} />
            </div>
            <div className="min-w-0">
              <p
                className={cn(
                  "truncate text-sm font-bold",
                  r.isYou ? "text-primary" : "text-light"
                )}
              >
                {r.name}
                {r.isYou && (
      ...
      <p className="mt-5 flex items-center justify-center gap-1.5 text-center text-[11px] text-gray/60">
        <TrendingUp size={12} />
        Tabla de ejemplo · se actualiza en vivo con cada partido.
      </p>
    </div>
  );
};
