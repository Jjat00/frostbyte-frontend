...
- Anything that FLOATS over the page (dialog, modal, bottom sheet, popover) uses `fb-sheet`, never `fb-card`: `fb-card` is glass and only works resting on the page background — over a table or a form it shows everything underneath and the text becomes unreadable.
- NEVER put a colored stripe on a card's top or side border (`border-t-4 border-<color>`, inline `borderTopColor`, `border-l-2` to flag a row). To identify an entity by color use a 6-10px dot next to its name, the icon, or the value itself; to make a whole card stand out use `fb-card--accent`, which tints the entire border faintly.
